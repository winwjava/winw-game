...
public class Indicator extends Quote {
	 ...
	 */
	protected static void computeZscore(List<Indicator> list) {
		double[] closeArray = new double[list.size()];
		for (int i = 0; i < list.size(); i++) {
			Indicator indicator = list.get(i);
			closeArray[i] = indicator.close;
			if (i < 20) {
				continue;
			}
			double std = Math.sqrt(StatUtils.populationVariance(Arrays.copyOfRange(closeArray, i - 20, i)));
			indicator.zscore = ((indicator.close - indicator.ma20) / std);
		}
	}
